...
class Get_messages(object):
	...
	def get_messages(self, qq, target_qq, cookie, db):
		...
		if r.content.decode('utf-8').find(u'您无权访问') > 0:
			...
			while True:
				url = 'https://h5.qzone.qq.com/proxy/domain/m.qzone.qq.com/cgi-bin/new/get_msgb?uin=' + qq.user + '&hostUin=' + target_qq + \
					'&start=' + str(start) + '&num=10&format=jsonp&inCharset=utf-8&outCharset=utf-8&g_tk=' + g_tk
				r = s.get(url, headers = header)
	...
	def operate_db_message(self, db, tablename, data):
		...
		for item in data['data']['commentList']:
			i = {'replyFlag': 0}
			if item['secret'] == 0:							#secret message
			...
			insert_data_sql = insert_data_sql.strip().replace(';','').replace("'",'').replace('%','').replace('\\','').replace('\r','').replace('\t','').replace('\n','')
	...
	def operate_db_reply(self, db, tablename, data):
		...
		db.Create_table(tablename, create_tb_sql)
		for item in data['data']['commentList']:
			if item.has_key('replyList') and item['replyList'] != []:
